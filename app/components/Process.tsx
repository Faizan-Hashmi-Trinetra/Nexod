...
const STEPS = [
    {
        id: 1, color: "#5857f9", title: "Discover",
        points: ["Business Model Analysis", "Bottleneck Identification", "AI Leverage Mapping", "Competitive Gap Review"],
    },
    {
        id: 2, color: "#9c34f0", title: "Design",
        points: ["Scalable System Blueprints", "Seamless UX Architecture", "Growth-Aligned Strategy", "Integration Spec Definition"],
    },
    {
        id: 3, color: "#fb5457", title: "Build",
        points: ["Production-Ready Platforms", "Real-World Testing", "Scalable Deployment", "Continuous Optimisation"],
    },
];
...
export default function Process() {
    ...
    useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            STEPS.forEach((step) => {
                const titleEl = document.querySelector<HTMLElement>(`#ptitle-${step.id}`);
                const descEl = document.querySelector<HTMLElement>(`#mob-desc-${step.id}`);
                const svgEl = document.querySelector<HTMLElement>(`#mob-svg-${step.id}`);

                if (titleEl) {
                    gsap.fromTo(titleEl,
                        { y: 36, opacity: 0 },
                        { y: 0, opacity: 1, duration: 0.85, ease: "power3.out", scrollTrigger: { trigger: titleEl, start: "top 88%", toggleActions: "play none none reset" } }
                    );
                }
                if (descEl) {
                    const items = descEl.querySelectorAll<HTMLElement>(".mob-point");
                    if (items.length) {
                        gsap.fromTo(items,
                            { y: 20, opacity: 0 },
                            { y: 0, opacity: 1, duration: 0.7, ease: "power3.out", stagger: 0.07, scrollTrigger: { trigger: descEl, start: "top 88%", toggleActions: "play none none reset" } }
                        );
                    }
                }
                if (svgEl) {
                    gsap.fromTo(svgEl,
                        ...
                        { y: 0, opacity: 1, scale: 1, duration: 0.9, ease: "power3.out", scrollTrigger: { trigger: svgEl, start: "top 90%", toggleActions: "play none none reset" } }
                    );
                }
            });
        });
    ...
    useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            ...
            ScrollTrigger.create({
                trigger: wrapper, start: "top top", end: `+=${totalScroll}`,
                pin: sticky, pinSpacing: true, anticipatePin: 1,
            });

            function showTitlesOnly() {
                titleEls.forEach((el) => el && gsap.to(el, { y: 0, opacity: 1, duration: 0.5, ease: "power3.out", overwrite: true }));
                descEls.forEach((el) => el && gsap.to(el, { height: 0, opacity: 0, duration: 0.3, ease: "power2.in", overwrite: true }));
                visualEls.forEach((el) => el && gsap.to(el, { opacity: 0, y: 24, duration: 0.3, ease: "power2.in", overwrite: true }));
            }
            function showStep(i: number) {
                visualEls.forEach((el, vi) => {
                    if (!el) return;
                    if (vi === i) gsap.to(el, { opacity: 1, y: 0, duration: 0.55, ease: "power3.out", overwrite: true });
                    else gsap.to(el, { opacity: 0, y: vi < i ? -24 : 24, duration: 0.35, ease: "power2.in", overwrite: true });
                });
                descEls.forEach((el, di) => {
                    if (!el) return;
                    if (di === i) gsap.to(el, { height: "auto", opacity: 1, duration: 0.55, ease: "power3.out", overwrite: true });
                    else gsap.to(el, { height: 0, opacity: 0, duration: 0.35, ease: "power2.in", overwrite: true });
                });
                titleEls.forEach((el, ti) => {
                    if (!el) return;
                    if (ti < i) gsap.to(el, { y: -60, opacity: 0, duration: 0.4, ease: "power2.in", overwrite: true });
                    else gsap.to(el, { y: 0, opacity: 1, duration: 0.55, ease: "power3.out", overwrite: true });
                });
            }

            const stepOffset = window.innerHeight;
            ScrollTrigger.create({
                trigger: wrapper, start: "top top", end: `+=${stepOffset - 10}`,
                onEnter: () => showTitlesOnly(), onEnterBack: () => showTitlesOnly(),
            });
            STEPS.forEach((_, i) => {
                ScrollTrigger.create({
                    trigger: wrapper,
                    ...
                    onEnter: () => showStep(i), onEnterBack: () => showStep(i),
                });
            });
        }, wrapper);
        return () => ctx.revert();
    ...
    return (
        <section id="process" style={{ background: "#fff" }}>
            ...
            {/* ── Section header ──────────────────────────────────────── */}
            <div style={{ maxWidth: 1280, margin: "0 auto", padding: isMobile ? "80px 24px 48px" : "160px 48px 80px" }}>
                <div style={{ maxWidth: 680 }}>
                    ...
                    <h2 style={{
                        fontSize: isMobile ? "clamp(38px,9vw,56px)" : "clamp(52px,6vw,80px)",
                        fontWeight: 700, letterSpacing: "-0.04em", lineHeight: 1.02,
                        color: "#0a0a0a", margin: 0, perspective: "800px",
                    }}>
                    ...
                    <p className="proc-subtext" style={{
                        ...
                        color: "#000", maxWidth: 480, margin: "24px 0 0 0", fontWeight: 400,
                    }}>
            ...
            {/* ══════════════════════════════════════════════════════════
                MOBILE LAYOUT
                CRITICAL: #ptitle-1/2/3 IDs + AnimatedBullet SVGs MUST
                exist in DOM — StoryCanvas uses "#ptitle-X svg" to find
                dot travel targets. Without them dots park at (0,0).
            ══════════════════════════════════════════════════════════ */}
            {isMobile && (
                <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 24px 80px" }}>
                    {STEPS.map((step, i) => (
                        <div key={step.id} style={{
                            ...
                            {/* ── Title row — ID required by StoryCanvas ── */}
                            {/*
                                #ptitle-${step.id} must contain an <svg> child.
                                AnimatedBullet renders the spinning ring SVG.
                                StoryCanvas finds it via "#ptitle-1 svg" selector.
                            */}
                            <div
                                id={`ptitle-${step.id}`}
                                style={{
                            ...
                            >
                                {/* AnimatedBullet — contains the SVG StoryCanvas targets */}
                                <AnimatedBullet color={step.color} dotIndex={i} />

                                <h3 style={{
                                    fontSize: "clamp(28px,7vw,36px)",
                                    fontWeight: 900, letterSpacing: "-0.03em",
                                    lineHeight: 1, color: "#0a0a0a", margin: 0,
                                }}>
                                    {step.title}
                                </h3>
                            </div>
                            ...
                            {/* ── Description ── */}
                            <div id={`mob-desc-${step.id}`}>
                                <div style={{ height: 1, background: "rgba(0,0,0,0.07)", marginBottom: 20 }} />
                                <ul style={{ listStyle: "none", margin: "0 0 32px 0", padding: 0, display: "flex", flexDirection: "column", gap: 14 }}>
                                    {step.points.map((point, pi) => (
                                        <li key={pi} className="mob-point" style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                                            <span style={{
                                                flexShrink: 0, marginTop: 7,
                                                width: 5, height: 5, borderRadius: "50%",
                                                background: step.color,
                                            }} />
                                            <span style={{ fontSize: 15, lineHeight: 1.65, color: "rgba(0,0,0,0.62)", fontWeight: 400 }}>
                                                {point}
                                            </span>
                                        </li>
                                    ))}
                            ...
                            {/* ── SVG Visual ── */}
                            <div id={`mob-svg-${step.id}`} style={{ display: "flex", justifyContent: "center" }}>
                                {step.id === 1 && <ResearchMappingSVG size={240} />}
                                {step.id === 2 && <SystemArchitectureSVG size={240} />}
                                {step.id === 3 && <DeploymentInfrastructureSVG size={240} />}
                            </div>
            ...
            {/* ══════════════════════════════════════════════════════════
                DESKTOP LAYOUT — original pinned scroll, unchanged
            ══════════════════════════════════════════════════════════ */}
            {!isMobile && (
                <div ref={wrapperRef}>
                    <div
                    ...
                    >
                        <div style={{ position: "relative" }}>
                            {STEPS.map((step, i) => (
                                <div key={step.id}>
                                    <div id={`ptitle-${step.id}`} style={{ display: "flex", alignItems: "center", gap: 20, marginBottom: 4 }}>
                                        <AnimatedBullet color={step.color} dotIndex={i} />
                                        <h3 style={{ fontSize: "clamp(44px,4.2vw,60px)", fontWeight: 900, letterSpacing: "-0.03em", lineHeight: 1, color: "#0a0a0a", margin: 0 }}>
                                            {step.title}
                                        </h3>
                                    </div>
                                    <div id={`pdesc-${step.id}`} style={{ overflow: "hidden", height: 0, opacity: 0 }}>
                                        ...
                                        <ul style={{ listStyle: "none", margin: 0, padding: "0 0 28px 68px", display: "flex", flexDirection: "column", gap: 14 }}>
                                            {step.points.map((point, pi) => (
                                                <li key={pi} style={{ display: "flex", alignItems: "center", gap: 14 }}>
                                                    ...
                                                    <span style={{ fontSize: 16, lineHeight: 1.65, color: "rgba(0,0,0,0.62)", fontWeight: 400 }}>{point}</span>
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
                        ...
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
                            <div style={{ position: "relative", width: 460, height: 460 }}>
                                <div id="pvisual-1" style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", opacity: 0 }}>
                                    <ResearchMappingSVG size={320} />
                                </div>
                                <div id="pvisual-2" style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", opacity: 0 }}>
                                    <SystemArchitectureSVG size={320} />
                                </div>
                                <div id="pvisual-3" style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", opacity: 0 }}>
                                    <DeploymentInfrastructureSVG size={320} />
                                </div>
                            </div>
                        </div>
                    </div>
...
function AnimatedBullet({ color, dotIndex }: { color: string; dotIndex: number }) {
    ...
    useEffect(() => {
        const fill = (on: boolean) => { wrapRef.current?.classList.toggle("bullet-filled", on); };
...
/* ── SVG Components ─────────────────────────────────────────────────────── */
function ResearchMappingSVG({ size }: { size: number }) {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 220 220" width={size} height={size}>
            <style>{`
            ...
            <line className="re" x1="110" y1="110" x2="60" y2="60" />
            <line className="re re2" x1="110" y1="110" x2="165" y2="72" />
            <line className="re re3" x1="110" y1="110" x2="170" y2="148" />
            <line className="re re4" x1="110" y1="110" x2="68" y2="155" />
            ...
            <circle className="rn" cx="60" cy="60" r="6" />
            <circle className="rn rn2" cx="165" cy="72" r="6" />
            <circle className="rn rn3" cx="170" cy="148" r="6" />
            <circle className="rn rn4" cx="68" cy="155" r="6" />
            <circle className="rn rn5" cx="48" cy="120" r="5" />
...
function SystemArchitectureSVG({ size }: { size: number }) {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 220 220" width={size} height={size}>
            <style>{`
            ...
            <rect className="ab" x="28" y="38" width="60" height="32" rx="3" />
            <rect className="ac" x="28" y="38" width="4" height="32" rx="1" />
            <text className="al" x="40" y="52">INPUT</text>
            <rect className="ab ab2" x="132" y="38" width="60" height="32" rx="3" />
            <rect className="ac ac2" x="132" y="38" width="4" height="32" rx="1" />
            <text className="al al2" x="144" y="52">PROCESS</text>
            <path className="aa" d="M88,54 L132,54" markerEnd="url(#sa-arrow)" />
            <rect className="ab ab3" x="80" y="100" width="60" height="32" rx="3" />
            <rect className="ac ac3" x="80" y="100" width="60" height="4" rx="1" />
            <text className="al al3" x="92" y="116">ORCHESTRATE</text>
            <path className="aa aa2" d="M162,70 L162,86 Q162,96 152,100" markerEnd="url(#sa-arrow)" />
            <path className="aa aa3" d="M58,70 L58,86 Q58,96 80,113" markerEnd="url(#sa-arrow)" />
            <rect className="ab ab4" x="80" y="158" width="60" height="30" rx="3" />
            <rect className="ac ac4" x="80" y="158" width="60" height="4" rx="1" />
            <text className="al al4" x="93" y="173">OUTPUT</text>
            <path className="aa aa4" d="M110,132 L110,158" markerEnd="url(#sa-arrow)" />
            <circle className="ad" cx="183" cy="42" r="3" />
            <circle className="ad ad2" cx="183" cy="54" r="3" />
            <circle className="ad ad3" cx="183" cy="66" r="3" />
            <defs>
                <marker id="sa-arrow" markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto">
                    <path d="M0,0 L0,6 L6,3 Z" fill="#011736" />
                </marker>
            </defs>
        </svg>
    );
...
function DeploymentInfrastructureSVG({ size }: { size: number }) {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 220 220" width={size} height={size}>
            <style>{`
            ...
            `}</style>
            <rect className="ds" x="22" y="68" width="50" height="14" rx="2" />
            <rect className="ds ds2" x="22" y="86" width="50" height="14" rx="2" />
            <rect className="ds ds3" x="22" y="104" width="50" height="14" rx="2" />
            <circle className="dl" cx="30" cy="75" r="3" />
            <circle className="dl dl2" cx="30" cy="93" r="3" />
            <circle className="dl dl3" cx="30" cy="111" r="3" />
            {[75, 93, 111].map((y, i) => <rect key={i} x="36" y={y - 2} width={24 - i * 4} height="4" rx="1" fill="#011736" opacity="0.15" />)}
            <text className="dlb" x="22" y="63">SRV-01</text>
            <text className="dlb dlb2" x="22" y="130">SRV-02</text>
            <path className="dp" d="M72,91 Q100,91 108,130" />
            <line className="dax" x1="108" y1="128" x2="200" y2="128" />
            <line className="dax" x1="108" y1="60" x2="108" y2="128" />
            <rect className="db" x="114" y="98" width="10" height="30" />
            <rect className="db db2" x="128" y="88" width="10" height="40" />
            <rect className="db db3" x="142" y="78" width="10" height="50" />
            <rect className="db db4" x="156" y="92" width="10" height="36" />
            <rect className="db db5" x="170" y="68" width="10" height="60" />
            <rect className="db db6" x="184" y="82" width="10" height="46" />
            <polyline
                points="119,98 133,88 147,78 161,92 175,68 189,72"
                fill="none" stroke="#7202BB" strokeWidth="1.5"
                strokeDasharray="100" strokeDashoffset="100"
                style={{ animation: "dln-d 1s ease 1.5s forwards" }}
            />
            <text className="dlb dlb3" x="108" y="140">DEPLOY CYCLES</text>
            <text x="170" y="62" fill="#7202BB" fontSize="7" fontFamily="monospace" opacity="0"
                style={{ animation: "cup-d .3s 2s ease forwards" }}>▲ 98%</text>
            <circle cx="189" cy="72" r="4" fill="#7202BB" opacity="0"
                style={{ animation: "ping-d 2s 2.2s ease-in-out infinite" }} />
        </svg>
    );
}